"""Runtime facade for the Observability Core module."""
...
class ObservabilityCoreConfig:
    """Runtime configuration for Observability Core."""
    ...
    @classmethod
    def from_mapping(cls, payload: Mapping[str, Any]) -> "ObservabilityCoreConfig":
...
class ObservabilityCore:
    """Primary facade exposing Observability Core capabilities."""

    def __init__(self, config: ObservabilityCoreConfig | None = None) -> None:
        self.config = config or ObservabilityCoreConfig()
...
_RUNTIME_SINGLETON: Optional[ObservabilityCore] = None
_RUNTIME_LOCK = threading.Lock()


def get_runtime(
    config: ObservabilityCoreConfig | None = None,
    *,
    refresh: bool = False,
) -> ObservabilityCore:
    """Return a cached ObservabilityCore runtime, optionally refreshing its configuration."""

    global _RUNTIME_SINGLETON
    with _RUNTIME_LOCK:
        if refresh or _RUNTIME_SINGLETON is None:
            _RUNTIME_SINGLETON = ObservabilityCore(config=config)
...
def register_fastapi(app: FastAPI, config: ObservabilityCoreConfig | None = None) -> APIRouter:
    """Attach the Observability Core router to the given FastAPI app."""

    # Lazy import to avoid circular imports at module import time.
    from .routers.observability_core import build_router
...
def register_metrics_endpoint(app: FastAPI, config: ObservabilityCoreConfig | None = None) -> APIRouter:
