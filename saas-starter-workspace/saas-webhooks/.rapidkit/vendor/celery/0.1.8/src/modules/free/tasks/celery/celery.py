...
class CeleryTaskRegistry:
    ...
    def task(self, *decorator_args: Any, **decorator_kwargs: Any) -> Callable[[Callable[..., Any]], Any]:
        if Celery is None:
            raise CeleryRuntimeError("Celery is required to register tasks. Install it via 'pip install celery[redis]'.")
...
def create_celery_app(config: CeleryAppConfig) -> Any:
    if Celery is None:
        raise CeleryRuntimeError("Celery is not installed. Install via 'pip install celery[redis]'.")
...
def load_config_from_env(prefix: str = DEFAULT_ENV_PREFIX, env: Mapping[str, str] | None = None) -> CeleryAppConfig:
    ...
    settings = CelerySettings.from_mapping({k: v for k, v in settings_payload.items() if v not in (None, "")})
...
def describe_celery(
    config: CeleryAppConfig | None = None,
    *,
    app: Any | None = None,
    include_tasks: bool = False,
) -> MutableMapping[str, Any]:
    """Return normalized metadata describing the Celery configuration."""

    resolved_config = config or CeleryAppConfig()
    ...
    if include_tasks and app is None:
        try:
            app = get_celery_app(resolved_config)
...
def get_celery_metadata(*, include_tasks: bool = False) -> MutableMapping[str, Any]:
    """Convenience helper returning metadata derived from environment configuration."""

    return describe_celery(load_config_from_env(), include_tasks=include_tasks)
