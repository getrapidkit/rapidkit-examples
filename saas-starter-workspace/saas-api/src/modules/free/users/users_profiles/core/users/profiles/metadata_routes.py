"""FastAPI routes exposing Users Profiles metadata and features."""
...
from src.modules.free.users.users_profiles.users_profiles import (
    MODULE_FEATURES,
    get_users_profiles_metadata as get_users_profiles_metadata_runtime,
)
...
async def get_users_profiles_metadata() -> Dict[str, object]:
    """Expose normalized metadata describing the Users Profiles configuration."""

    metadata = get_users_profiles_metadata_runtime()
    return dict(metadata)


@router.get("/features", status_code=status.HTTP_200_OK)
async def list_users_profiles_features() -> Dict[str, object]:
    """Return the list of advertised capabilities for the Users Profiles module."""
...
async def get_users_profiles_health() -> Dict[str, object]:
    """Return health snapshot for the Users Profiles subsystem."""
...
def register_users_profiles_routes(app: FastAPI) -> None:
    """Attach the Users Profiles metadata routes to a FastAPI application."""
