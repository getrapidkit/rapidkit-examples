...
def _call_launcher(command: str, *args: str, env: dict | None = None) -> int:
    ...
    if not launcher.exists():
        print("❌ Local launcher not found (.rapidkit/rapidkit). Run `rapidkit init` or create .rapidkit/rapidkit.")
        return 127
...
def dev(port: int = 8000, host: str = "0.0.0.0", allow_global_runtime: bool = False) -> None:
    ...
    def _find_available_port(start_port: int, host: str = "0.0.0.0", max_attempts: int = 50) -> int | None:
        candidate = max(1, start_port)
        for _ in range(max_attempts):
            if not _port_in_use(candidate, host):
                return candidate
            candidate += 1
        return None
    ...
    if ptype == "python":
        selected_port = port
        if _port_in_use(selected_port, host):
            fallback = _find_available_port(selected_port + 1, host)
            if fallback is None:
                print(f"❌ Port {selected_port} is busy and no free port was found nearby.")
                sys.exit(1)
            print(f"⚠️  Port {selected_port} is in use. Switching to {fallback} automatically.")
        ...
        if venv_dir.exists():
            if not _venv_has_uvicorn(venv_dir):
                ...
                print("💡 Run 'rapidkit init' to (re)bootstrap the project and install dependencies.")
                sys.exit(1)
        else:
            ...
            if shutil.which("uvicorn") is None:
                print("❌ No uvicorn executable found on PATH and no .venv present - nothing to run.")
                print("")
                print("💡 Either install uvicorn globally (pip install uvicorn) or run:")
                print("")
                print("  rapidkit init")
                print("")
                sys.exit(1)
        ...
        include_tests = _os.getenv("RAPIDKIT_DEV_INCLUDE_TESTS", "").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        }
        if include_tests and (root / "tests").exists():
            reload_args.extend(["--reload-dir", "tests"])

        force_polling = _os.getenv("RAPIDKIT_DEV_FORCE_POLLING", "").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        }
        dev_env = {"WATCHFILES_FORCE_POLLING": "true"} if force_polling else None
        if force_polling:
            print("⚠️  RAPIDKIT_DEV_FORCE_POLLING is enabled (reload uses polling mode).")
        ...
        if not (root / "node_modules").exists():
            print("❌ Project dependencies appear to be missing (node_modules not found).")
            print("")
            print("💡 Run the following to bootstrap the project and install dependencies:")
            print("")
            print("  rapidkit init")
            print("")
        ...
        if _port_in_use(selected_port, host):
            ...
            if fallback is None:
                print(f"❌ Port {selected_port} is busy and no free port was found nearby.")
                print("")
                print("💡 Run 'rapidkit init' if the project is not bootstrapped yet.")
                sys.exit(1)
            print(f"⚠️  Port {selected_port} is in use. Switching to {fallback} automatically.")
            selected_port = fallback
        ...
        rc = _call_launcher("dev", env=env)
        if rc != 0:
            sys.exit(rc)
    else:
        print("❌ Unknown project type. Ensure pyproject.toml or package.json exists.")
        sys.exit(1)
...
def init() -> None:
    ...
    if ptype == "python":
        ...
        if rc != 0:
            print("❌ Failed to run 'rapidkit init' locally. Try running it via the global 'rapidkit init' too.")
            sys.exit(rc)
    elif ptype == "node":
        rc = _call_launcher("init")
        if rc != 0:
            print("❌ Failed to install Node dependencies via package manager.")
            sys.exit(rc)
    else:
        print("❌ Unknown project type. Ensure pyproject.toml or package.json exists.")
        sys.exit(1)
...
def start(port: int = 8000, host: str = "0.0.0.0", allow_global_runtime: bool = False) -> None:
    ...
    if ptype == "python":
        ...
        dist_dir = root / "dist"
        dist_main = dist_dir / "main"
        dist_main_js = dist_dir / "main.js"
        if not dist_main.exists() and not dist_main_js.exists():
            print("❌ Build output not found (dist/main).")
            print("")
            print("💡 Running build before start:")
            print("")
            print("  rapidkit build")
            rc = _call_launcher("build")
            if rc != 0:
                sys.exit(rc)
            if not dist_main.exists() and not dist_main_js.exists():
                print("❌ Build output still missing after build (dist/main).")
                print("💡 Ensure the build script outputs dist/main or dist/main.js")
                sys.exit(1)

        env = dict()
        env.update({"PORT": str(port), "HOST": host})
        rc = _call_launcher("start", env=env)
        if rc != 0:
            sys.exit(rc)
...
def help_cmd() -> None:
    _print_banner("📚", "Project Commands (callable)")
