"""Runtime facade for the Security Headers module."""
...
class SecurityHeadersConfig:
    """Declarative configuration for Security Headers middleware."""
...
def _expected_headers(config: SecurityHeadersConfig) -> Iterable[str]:
...
def evaluate_completeness(
    config: SecurityHeadersConfig, headers: Mapping[str, str]
) -> SecurityHeadersMetrics:
    expected = set(_expected_headers(config))
    present = set(headers)
    missing = sorted(expected - present)
    return SecurityHeadersMetrics(
...
def _build_hsts_value(config: SecurityHeadersConfig) -> str:
...
class SecurityHeaders:
    """Primary facade exposing Security Headers capabilities."""
    ...
    @property
    def config(self) -> SecurityHeadersConfig:
        return self._config
