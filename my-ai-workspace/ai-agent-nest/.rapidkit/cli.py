...
def dev(port: int = 8000, host: str = "0.0.0.0", allow_global_runtime: bool = False) -> None:
    ...
    if ptype == "python":
        ...
        if _port_in_use(port, host):
            ...
            alt = port + 1
            print(f"  • Try a different port: rapidkit dev -p {alt}")
            print(f"  • Or set the PORT env and run again: PORT={alt} rapidkit dev")
            print("  • Stop the process currently using that port and retry")
            print("")
